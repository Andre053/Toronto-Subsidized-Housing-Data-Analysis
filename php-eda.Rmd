...
TCHC is the largest provider with 16 buildings, followed by Public/Non-profit (PNP) providers running 8, and CO-OP providers running 2. Only TCHC and Villa Otthon run more than 1 building. 
...
Add provider type into df_unit
```{r}
...
Of the 26 buildings, how do the values vary? Note, units can house differing amounts of people.
```{r}
# all buildings
df_units |>
...
71 of the unit entries have subsidized housing, the others are when a unit size is offered but only for market housing. 
...
Does a particular provider type create more of certain subsidized units?
```{r}
check_var <- function(provider) {
  print("In function")
}

# 1. Need subsidized unit data, classified by provider type
# 2. Bar plot with % of units 
